...
{
    public partial class frmMusicType : Form
    {
        ...
        public void getZong()
        {
            ...
            try
            {
                ...
                if (!leixing.Equals("0"))
                {
                    tiaojian += " and MusicTypeID= " + leixing;
                }
        ...
        public void showMusic()
        {
            ...
            try
            {
                ...
                if (!leixing.Equals("0"))
                {
                    tiaojian += " and MusicTypeTable.MusicTypeID= " + leixing;
                }
                ...
                string sql = "select top " + pageSize + " MusicID,MusicName,Singer.SingerName,Music.SingerID,MusicTypeTable.MusicTypeName,'点歌' as dg from Music  INNER JOIN Singer ON Music.SingerID = Singer.SingerID INNER JOIN MusicTypeTable ON Music.MusicTypeID = MusicTypeTable.MusicTypeID where SingerDisplay='True' and MusicID not in(select top " + (pageSize * (pageNow - 1)) + " MusicID from Music where MusicDisplay='True'" + tiaojian + ")" + tiaojian;
                SqlDataReader reader = DBHelper.GetExecuteReader(sql);
                for (int i = this.panel10.Controls.Count - 1; i >= 0; i--)
                {
                    if (reader.Read())
                    {
                        this.panel10.Controls[i].Controls[3].Text = reader["MusicName"].ToString();
                        this.panel10.Controls[i].Controls[2].Text = reader["SingerName"].ToString();
                        this.panel10.Controls[i].Controls[2].Tag = reader["SingerID"].ToString();
                        this.panel10.Controls[i].Controls[1].Text = reader["MusicTypeName"].ToString();
                    ...
                    //如果没读到信息就把歌框隐藏
                    if (this.panel10.Controls[i].Controls[3].Text.Trim().Equals(""))
                    {
        ...
        //远程调用类型
        public void openType(string rType)
        {
            leixing = rType;
            pageNow = 1;
            getZong();//获取总页数
            ...
            {
                if (panel8.Controls[i].Tag.Equals(leixing))
                {
                    panel8.Controls[i].ForeColor = Color.RoyalBlue;
                }
                else
                {
                    panel8.Controls[i].ForeColor = Color.Black;
                }
            }
        }
        //传值点歌
